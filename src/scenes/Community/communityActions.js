...
export const COMMUNITY_SCENE_MARK_CLOSE_QUESTION_SUCCESS = 'COMMUNITY_SCENE_MARK_CLOSE_QUESTION_SUCCESS';
export const COMMUNITY_SCENE_MARK_CLOSE_QUESTION_ERROR = 'COMMUNITY_SCENE_MARK_CLOSE_QUESTION_ERROR';
...
export const findQuestions = (questionId) => {
  return (dispatch, getState) => {
    ...
    if(questionId) {
      ...
      return questionApi.getById(token, questionId)
      ...
      const {currentPosition: {coords: {latitude: lat, longitude: lng}}} = getState().app;
      ...
      return questionApi.find(token, lat, lng, radius, radiusUnit)
        .then((resp) => dispatch(appActions.processApiResponse(resp)))
        .then(
          (questions) => {
            let question = {};
...
export const findAnswers = () => {
  return (dispatch, getState) => {
    ...
    return questionApi.findAnswers(token, questionId)
      .then((resp) => dispatch(appActions.processApiResponse(resp)))
      .then(
        (answers) => {
          dispatch({
...
export const createAnswer = () => {
  return (dispatch, getState) => {
    ...
    return questionApi.createAnswer(token, questionId, data)
      .then((resp) => dispatch(appActions.processApiResponse(resp)))
      .then(
        (answer) => {
          dispatch({
...
export const markCloseQuestion = () => {
  return (dispatch, getState) => {
    ...
    return questionApi.update(token, questionId, data)
      .then((resp) => dispatch(appActions.processApiResponse(resp)))
      .then(
        (question) => {
          dispatch({
