...
export const CREATE_POST_SCENE_CHANGE_FORM_VALUE =
...
export const CREATE_POST_SCENE_CREATE_SUCCESS =
...
export const CREATE_POST_SCENE_GET_GEOCODE_DATA =
...
export const CREATE_POST_SCENE_GET_GEOCODE_DATA_ERROR =
	"CREATE_POST_SCENE_GET_GEOCODE_DATA_ERROR";
export const CREATE_POST_SCENE_SET_CURRENT_LOCATION =
	...
	return (dispatch, getState) => {
		...
		return postApi
			...
			.then(post => {
                ...
                dispatch(
                    // navigationActions.changeScene("myPosts", {
                    // 	marketType: "free"
                    // })
                    navigationActions.resetScene("AuthFlowStack", {})
                );
		...
		Geocoder.geocodePosition(position)
			.then(data => {
				...
				if (isPositionAvailable) {
					...
					dispatch(
						appActions.onMessage(t(dictionary.gettingLocationError))
					);
				}
			})
			.catch(err => {
		...
		Geocoder.geocodeAddress(address)
			.then(data => {
				...
				if (isPositionAvailable) {
					...
					// prompt user that address is not available
					dispatch(
						appActions.onMessage(t(dictionary.locationInvalid))
					);
				}
			})
			.catch(err => {
